...
class Core_Api_Validate_Auth_DoesNotExists extends Zend_Validate_Abstract {
    const AUTH_DOES_NOT_EXISTS = 'authDoesNotExists';

    protected $_messageTemplates = array(
        self::AUTH_DOES_NOT_EXISTS => 'The username supplied is not a valid username.'
    );
    ...
    public function isValid($value, $context = null) {
		...
		// check to see if the user does not exists
		if (!$authAdapter->userExists()) {
		    $error=true;
		    $this->_error(self::AUTH_DOES_NOT_EXISTS);
		}
        return !$error;
    }
}
